...
inline bool bisearch(uint32_t ucs, const Range *interval, int max)
{
  ...
  while (max >= min)
  {
    ...
    if (ucs > interval[mid].last)
      min = mid + 1;
    else if (ucs < interval[mid].first)
      max = mid - 1;
    else
      return true;
  }
...
{
public:
  Char(uint32_t c) : cp(c) {}
  Char(char c) : cp(static_cast<unsigned char>(c)) {}

  bool operator==(Char other) const { return cp == other.cp; }
  bool operator!=(Char other) const { return cp != other.cp; }
  ...
  bool isAscii() const { return cp <= 0x7F; }
  bool isControl() const { return (cp <= 0x1F) || (cp >= 0x7F && cp <= 0x9F); }
  bool isDigit() const { return cp >= '0' && cp <= '9'; }
  bool isAlpha() const
  {
    return (cp >= 'a' && cp <= 'z') || (cp >= 'A' && cp <= 'Z');
  }

  // returns the display width of the unicode character
  int width() const
  {
    ...
    if (bisearch(cp, combining, sizeof(combining) / sizeof(combining[0]) - 1))
    {
      return 0;
    }

    if (bisearch(cp, wide, sizeof(wide) / sizeof(wide[0]) - 1))
    {
      return 2;
    }
  ...
  uint32_t value() const { return cp; }
  bool isValid() const
  {
